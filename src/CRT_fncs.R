...
# https://github.com/adriaaulaICM/bbmo_niche_sea/blob/master/src/util/crt_functions.R
####
library(moments)

calculate_abund_prevalen.df <- function(physeq){
  
    # Check if taxa are rows; transpose if needed
    ...
    # Add sample IDs (optional, for clarity)
    ...
    # Summarize by ASV
    abundance.prevalence.df <- abundance_df %>%
    group_by(asv) %>%
    summarize(
        ocurrence = sum(Abundance > 0),                 # Count of samples where ASV is present
        ocurrence_more1 = sum(Abundance >= 0.01),       # Count of samples with abundance >= 0.01
        mean.reads = mean(Abundance),                  # Mean abundance of ASV across samples
        .groups = 'drop'
    ) %>%
    mutate(
        prevalence = ocurrence / nsamples(physeq),       # Prevalence calculation
        presence = ifelse(ocurrence_more1 > 0, 'abundant', 'rare'), # Categorize ASV as abundant/rare
        behavior = case_when(                           # Classify behavior
        prevalence >= 0.75 ~ 'Broad',
...
incorporate_crt_results <- function(prev.df, physeq, string){
  taxa.hyp.crt <- prev.df %>% as.data.frame() %>%
    filter( ocurrence_more1 > 0, behavior %in% c('Narrow', 'Intermediate')) %>% 
    pull(asv)
  
  table.crt <- prune_taxa(taxa_names(physeq) %in% taxa.hyp.crt, physeq) %>%
    otu_table() %>% 
    as(. , 'matrix') %>% 
    t() %>% 
    data.frame()
...
SimpleRareToPrev.f=function(otu_fp,outitle,abund_thresh = 0.005, abund_thresh_ALL=FALSE,b_thresh = 0.90, rdp_lastcol=TRUE){
  ...
  write.table(out.filter, paste("data/ResultsFile_ConditionallyRareOTUID_",outitle, abund_thresh, "_", b_thresh, "_", at, ".txt", sep=""), quote=FALSE, sep="\t", row.names=FALSE)
